class LinebotController < ApplicationController
  ...
  def callback
    ...
    events.each { |event|
      id = ENV["GURUNAVI_KEY_ID"] 
      if event.message['text'] != nil
        freeword = event.message['text'] #ここでLINEで送った文章を取得
        
        result = `curl -X GET http://api.gnavi.co.jp/RestSearchAPI/20150630/?keyid=#{id}'&'format=json'&'freeword=#{freeword}`
      else
        ...
        result = `curl -X GET http://api.gnavi.co.jp/RestSearchAPI/20150630/?keyid=#{id}'&'format=json'&'latitude=#{latitude}'&'longitude=#{longitude}`
      end
      #ここでぐるなびAPIを叩く
